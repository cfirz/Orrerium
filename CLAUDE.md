...
**BrainOS is a local-first dashboard over a markdown knowledge vault**: an interactive graph, a
projects board, the inbox view, and ask-your-brain. It ships with a demo vault in
`starter-vault/` and points at it by default; `BRAINOS_VAULT` or `config.json` selects a real
...
`brainos.bat` (Windows) and `brainos.sh` (macOS/Linux) are the human double-click paths (server +
...
- **Never write the vault.** BrainOS reads the vault live over recursive `fs.watch` and stays
