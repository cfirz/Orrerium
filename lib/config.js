...
const DEFAULTS = {
  // the bundled demo vault; point BRAINOS_VAULT or config.json at your own
...
export function loadConfig() {
  ...
  if (process.env.BRAINOS_VAULT) config.vaultPath = process.env.BRAINOS_VAULT;
  config.dataDir = path.join(ROOT, 'data'); // BrainOS-owned writable state; the vault stays read-only
  ...
  if (!existsSync(config.vaultPath) || !statSync(config.vaultPath).isDirectory()) {
    throw new Error(
      ...
      'Set it via the BRAINOS_VAULT environment variable, "vaultPath" in config.json ' +
