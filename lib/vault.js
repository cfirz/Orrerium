...
// folder name -> note type; config.json's folderTypes replaces this map, but
// the values must stay within these five types - the UI's rings, colours and
// boards are keyed on them
...
export function parseVault(vaultPath, opts = {}) {
  ...
  for (const relPath of listMarkdownFiles(vaultPath, excludeDirs)) {
    try {
      notes.push(parseNote(vaultPath, relPath, opts));
    } catch (err) {
...
export function parseNote(vaultPath, relPath, opts = {}) {
  ...
  return {
    ...
    path: relPath,
    folder,
    type: typeOf(fields, folder, opts.folderTypes),
...
function typeOf(fields, folder, folderTypes = DEFAULT_FOLDER_TYPES) {
  const t = str(fields.type);
  if (t) return t;
  return folder ? (folderTypes[folder] ?? 'root') : 'root';
}
